...
<html:html lang="true">
  ...
  --%>
      <script language="javascript">
    		...
    		function check()
			{
			  var appointminute=document.all['pworkset.appointminute'].value;
			  var zuhe=document.all['pworkset.zuhe'].value;
			  
			  if( zuhe == ""){
			      alert('地点类型不能为空');
			      document.all['pworkset.zuhe'].focus();
			      return false;
			  }if( appointminute == ""){
			      alert('指定时间不能为空');
			      document.all['pworkset.appointminute'].focus();
			      return false;
			  }
			  return true;
			}
			...
			function del()
			{
				...
				var v1 = document.getElementById("pworkset.zuhe").value;
				if (confirm("确定要删除吗？"))
				
				window.parent.frames['arightFrame'].location.href="page_range_topleft.do?method=deletetype&zuhe1="+v1;
			...
			function DeleteSomeLine(form)
			{
			    ...
			    if (SelectedCounts(form,"checkId")>0){
				    if(confirm("删除已选中的记录,将无法恢复！\n     确定要这样做吗？"))
				    {
				     ...
				     window.location.href="pworkset.do?method=deleteSite&lid="+str+"&type1=<%=type%>&sid1=<%=sid%>&aname=<%=name%>";   
					 return true;
				    }
				   }else{
					    alert("请先选择要删除的记录!");
  ...
  <body bgColor="white" background="Image/right.gif" >
    ...
    <html:form action="/pworkset?method=addSite" target="topFrame" onsubmit="return check()" focus="pworkset.zuhe" method="post"  >
    <td ><font size="2" color="red">地点设置--指定时间到达指定地点:</font></td>
    <br />
    <td><font size="2" color="blue">人员：<%=name%></font></td>
    <td>&nbsp;&nbsp;</td>
    <td><font size="2" color="blue">工种：<%=type%></font></td>
    <table width="320">
