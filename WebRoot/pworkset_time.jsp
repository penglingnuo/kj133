  ...
  --%>
      <script language="javascript">
    		...
    		function check()
			{
			  ...
			  var bantype=document.all['pworkset.bantype'].value;
...
//				window.parent.frames['arightFrame'].location.href="pworkset.do?method=deleteTime&type1=<%=type%>&sid=<%=sid%>";
				location.href="pworkset.do?method=deleteTime&type1=<%=type%>&sid=<%=sid%>";
			...
			function DeleteSomeLine(form)
			{
			    ...
			    if (SelectedCounts(form,"checkId")>0){
				    if(confirm("删除已选中的记录,将无法恢复！\n     确定要这样做吗？"))
				    {
				     ...
				     location.href="pworkset.do?method=deleteTime&lid="+str+"&type1=<%=type%>&aname=<%=name%>&id=<%=sid%>";   
					 return true;
				    }
				   }else{
					    alert("请先选择要删除的记录!");
  ...
  <body bgColor="white" background="Image/right.gif" >
    ...
    --%>
    <html:form action="/pworkset?method=addTime" target="downFrame" onsubmit="return check()" focus="pworkset.mintime" method="post"  >
   <td ><font size="2" color="red">时间设置:</font></td>
    ...
    <tr>
           ...
           <td><html:submit>员工设置</html:submit>&nbsp;</td>
