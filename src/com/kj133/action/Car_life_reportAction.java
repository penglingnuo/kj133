...
import com.kj133.entity.bo.Car_life_reportBO;
 ...
 */
public class Car_life_reportAction extends DispatchAction {
	...
	public ActionForward init(ActionMapping mapping, ActionForm form,
		...
		Car_life_reportBO bo = new Car_life_reportBO();
		DynaActionForm dy = (DynaActionForm) form;
		try {
			Search_car_life_report car_life_report = (Search_car_life_report) dy
					.get("car_life_report");
			Ouser user = (Ouser) request.getSession().getAttribute("user");
			String userId = user.getUserid();

			List relist = bo.getList(car_life_report, userId);
			...
			if (relist.size() == 0) {
				...
				request.getSession().setAttribute("car_life_report",car_life_report);
				request.getSession().setAttribute("car_life_reportPrint",relist);

			}
			List type = lib.carType();
			request.setAttribute("workType_list", type);
			dy.set("car_life_report", car_life_report);
		} catch (Exception e) {
			log.error("==车辆班次日月明细表==",e);
		}
	...
	public ActionForward getWordlib(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Search_car_life_report car_life_report = (Search_car_life_report) dy.get("car_life_report");
			car_life_report.setDaytime(daytime);
			car_life_report.setMonthtime(monthtime);
			car_life_report.setIsDayOrMonth("day");
