...
import com.kj133.entity.bo.DownWellCount_menologyBO;
 ...
 */
public class DownWellCount_menologyAction extends DispatchAction {
	...
	public ActionForward init(
	    ...
	    DownWellCount_menologyBO bo=new DownWellCount_menologyBO();
        DynaActionForm dy=(DynaActionForm)form;
        try{
        	Ouser user=(Ouser)request.getSession().getAttribute("user");
        	Search_downWellCount_menology downWellCount_menology=(Search_downWellCount_menology)dy.get("downWellCount_menology");

        	request.getSession().setAttribute("riqi", downWellCount_menology.getStime());
        	List list=bo.getList(downWellCount_menology,user.getUserid());
        	request.setAttribute("listCount", list.size());
			if (list != null && list.size()>0) {
				request.setAttribute("relist", list);
				request.getSession().setAttribute("downWellCount_menologyPrint", list);
				request.getSession().setAttribute("downWellCount_menology", downWellCount_menology);
			} else {
        	...
        	dy.set("downWellCount_menology",downWellCount_menology);
        }catch(Exception e){
        	log.error("==入井次数月报==",e);
        }
	...
	public ActionForward initialization(
	        ...
	        try{
			    ...
			    downWellCount_menology.setStime(etime);
			    Ouser user=(Ouser)request.getSession().getAttribute("user");
