...
public class Department_dailyAction extends DispatchAction {
	...
	public ActionForward viewPrint(
			...
			try{
	 			...
	 			// map.put("title"," 部门考勤日报");
				 map.put("riqi", request.getSession().getAttribute("riqi"));
				/* map.put("count", "序号");
				 map.put("gro", "部门");
				 map.put("allperson", "总人数");
				 map.put("zaocount", "早班");
				 map.put("zhongcount", "中班");
				 map.put("wancount", "晚班");
				 map.put("allcount", "总计");
				 map.put("xiucount", "休班");
				 map.put("addcount", "其中加班");*/
	...
	public ActionForward print(
			...
			else
			{
				...
				out.println("<span class=\"bold\">department_daily.Jasper文件不存在或解析错误！</span>");
				out.println("</body>");
				out.println("</html>");
			}			
		 return null;
