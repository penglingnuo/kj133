...
import com.kj133.entity.bo.Emphases_reportBO;
 ...
 */
public class Emphases_reportAction extends DispatchAction {
	...
	public ActionForward init(
		...
		Emphases_reportBO bo=new Emphases_reportBO();
		DynaActionForm dy=(DynaActionForm)form;
		try{
			Search_emphases_report check=(Search_emphases_report)dy.get("emphases_report");
        	...
        	request.getSession().setAttribute("emphases_report", check);
        	request.getSession().setAttribute("emphases_reportPrint", relist);
		    List gro=lib.grolist(user.getUserid()); 
			request.setAttribute("gro_list",gro);
			


		    dy.set("emphases_report",check);
		}catch(Exception e){
			log.error("==重点岗位日/月明细报==",e); 
		}
