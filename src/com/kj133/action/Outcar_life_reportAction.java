...
import com.kj133.entity.bo.Outcar_life_reportBO;
 ...
 */
public class Outcar_life_reportAction extends DispatchAction {
	...
	public ActionForward init(ActionMapping mapping, ActionForm form,
		...
		Outcar_life_reportBO bo = new Outcar_life_reportBO();
		DynaActionForm dy = (DynaActionForm) form;
		try {
			Search_outcar_life_report outcar_life_report = (Search_outcar_life_report) dy
					.get("outcar_life_report");
			Ouser user = (Ouser) request.getSession().getAttribute("user");
			String userId = user.getUserid();

			List relist = bo.getList(outcar_life_report, userId);
			...
			if (relist.size() == 0) {
				...
				request.getSession().setAttribute("outcar_life_report",outcar_life_report);
				request.getSession().setAttribute("outcar_life_reportPrint",relist);
...
//			request.setAttribute("pagination", pagination);//分页
			...
			dy.set("outcar_life_report", outcar_life_report);//重新把值设置回页面
		} catch (Exception e) {
//			e.printStackTrace();
			log.error("==车辆出车日/月报==",e);
		}
	 ...
	 * 初始化时间和班组
	 */
	public ActionForward getWordlib(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		DynaActionForm dy = (DynaActionForm) form;
		try {
			...
			Search_outcar_life_report outcar_life_report = (Search_outcar_life_report) dy
					.get("outcar_life_report");
			outcar_life_report.setDaytime(daytime);
			outcar_life_report.setMonthtime(monthtime);
			outcar_life_report.setIsDayOrMonth("day");


			request.setAttribute("listCount", "0");

			dy.set("outcar_life_report", outcar_life_report);
	...
	public ActionForward viewPrint(ActionMapping mapping, ActionForm form,
		...
		try {
			List list = (List) request.getSession().getAttribute(
					"employee_check_dailyPrint");
			File reportFile = new File(context
					.getRealPath("/reports/employee_check_daily.jasper"));
			...
			/*
			 * map.put("count", "序号"); map.put("name", "姓名"); map.put("group",
			 * "单位"); map.put("worktype", "工种"); map.put("zaoban", "早班");
			 * map.put("zhongban", "中班"); map.put("wanban", "晚班");
			 * map.put("downtime", "下井时间"); map.put("worktime", "工作时长");
			 */
	...
	public ActionForward print(ActionMapping mapping, ActionForm form,
		...
		if (jasperPrint != null) {
			...
			out
					.println("<span class=\"bold\">employee_check_daily.Jasper文件不存在或解析错误！</span>");
			out.println("</body>");
			out.println("</html>");
		}
		return null;
