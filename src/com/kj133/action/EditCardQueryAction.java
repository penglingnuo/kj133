 ...
 */
public class EditCardQueryAction extends DispatchAction {
	/*
	 * Generated Methods
	 */
	...
	public ActionForward getList(
		...
		try{
				  ...
				  pagination.setCount(100);//每页显示100条记录
			    ...
			    request.setAttribute("listCount", list.size());//在页面上判断打印预览按钮是否可用
			    if(list.size()==0){
			    	   ActionMessages messages=new ActionMessages();
				       messages.add(ActionMessages.GLOBAL_MESSAGE,new ActionMessage("no.count"));
				       this.saveMessages(request,messages);
			    }else{			    
			    	   request.setAttribute("editCardQueryList",list);
			    	   request.getSession().setAttribute("editCardQueryPrint",relist);
			    }
			...
			return mapping.findForward("qcard");
	}
...
//	导出Excel
	public ActionForward doOpenExcel(
			...
			try{
				...
				response.setHeader("content-disposition",
						"attachment;filename=chaxunkaxinxi.xls");
				// 定义输出类型
				response.setContentType("APPLICATION/msexcel");		
				WritableWorkbook wwb=Workbook.createWorkbook(os);
				WritableSheet ws =wwb.createSheet("查询卡信息",10);
				//	设置部分列的宽度
				...
				List list = (ArrayList)request.getSession().getAttribute("editCardQueryPrint");
				//	填充数据
				if(list.size()>0){
				...
				log.error("==查询卡信息--导出ecxel==",e);
			}
			catch(WriteException ex){
				log.error("==查询卡信息--导出ecxel==",ex);
			}
			catch(IOException e){
				log.error("==查询卡信息--导出ecxel==",e);
			}
	...
	public ActionForward getWorktype(
			...
			try{
				 ...
				 request.setAttribute("listCount", "0");//第一次的时候，打印预览按钮不可用
	...
	public ActionForward viewPrint(
			...
			try{
				 List list=(List)request.getSession().getAttribute("editCardQueryListView");         
