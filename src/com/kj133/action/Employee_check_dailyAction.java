...
import com.kj133.util.*;
import com.kj133.entity.Ouser;
import com.kj133.entity.Search_Employee_check_daily;
import com.kj133.entity.bo.Employee_check_dailyBO;
 ...
 */
public class Employee_check_dailyAction extends DispatchAction {
	...
	public ActionForward init(
		...
		Employee_check_dailyBO bo=new Employee_check_dailyBO();
		DynaActionForm dy=(DynaActionForm)form;
		Global go=new Global();
		try{
			Search_Employee_check_daily check=(Search_Employee_check_daily)dy.get("employee");
			request.getSession().setAttribute("riqi", check.getStime());
		    Ouser user=(Ouser)request.getSession().getAttribute("user");
        	String userId=user.getUserid();
        	String time=go.getDay(check.getStime(), -1);
        	List relist= bo.getList(time,check,userId);
        	...
        	if( relist.size()== 0){
     		...
     		request.setAttribute("emp",relist);
			request.getSession().setAttribute("employee_check_daily", check);
			request.getSession().setAttribute("employee_check_dailyPrint", relist);
     	 }
		    List gro=lib.grolist(user.getUserid()); 
			request.setAttribute("gro_list",gro);
			
		    dy.set("employee",check);
		}catch(Exception e){
			log.error("==员工考勤日报==",e);
		}
