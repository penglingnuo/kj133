...
//				  System.out.println("sb=" +sb.toString());
	 ...
	 */
		...
		public List count(Search_WorkAtt workatt) throws Exception{
	         ...
	         sb.append(" where w.downtime>=getdate()-2  and w.uptime is null  ");
	         sb.append(" and le.starttime >= (select MAX(starttime) - 2.0 / 1440 from locatedata_everyTimeMod) ");
	         sb.append(" and le.starttime>getDate()-5.0 / 1440 ");
	         ...
	         try{
				  engine=EngineFactory.getEngine("test");
				  Query query=engine.getQuery();
				  relist=query.getResults(sb.toString(),param.toArray(),WorkAttVO.class);
				  engine.commit();
			  }catch(Exception e){
					engine.rollback();
	  	    	log.error(e);
	...
	public List init(Search_WorkAtt workatt, Pagination pagination) throws Exception{
         ...
         try{
			  engine=EngineFactory.getEngine("test");
			  Query query=engine.getQuery();
			  relist=query.getResults(sb.toString(),param.toArray(),WorkAttVO.class,pagination);
			  engine.commit();
		  }catch(Exception e){
				engine.rollback();
  	    	log.error(e);
